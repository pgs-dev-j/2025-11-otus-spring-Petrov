...
class BookRepositoryTest {
    ...
    void shouldReturnCorrectBookById(Book expectedTemplate) {
        Author author = new Author(0, expectedTemplate.getAuthor().getFullName());
        List<Genre> genres = expectedTemplate.getGenres().stream()
                .map(g -> new Genre(0, g.getName()))
                .toList();

        Author persistedAuthor = em.persist(author);
        List<Genre> persistedGenres = genres.stream()
                .map(em::persist)
                .toList();
